...
1.  **拉新/破冰 (Acquisition)**:
    - **逻辑**：反向操作，禁止普通叫卖。
    - **话术特征**：“**XX你先别拍！**”、“手停一下”、“别划走，划走就亏了”。
    - **目的**：利用损失厌恶，强制制造停留。

2.  **塑品/价值 (Value Building)**:
    - **逻辑**：痛点 + 卖点 + 场景。
    - **话术特征**：讲清楚为什么值这个钱，解决什么问题。

3.  **保障/信任 (Assurance & Trust)**:
    - **逻辑**：基于人性数据库的信任建立。
    - **话术特征**：“我为什么给这个价（理由）”、“售后我包了”、“运费险我出了”、“不好吃你退给我”。
    - **目的**：消除下单前的最后顾虑。

4.  **逼单/收割 (Closing)**:
    - **逻辑**：稀缺性 + 紧迫感。
    - **话术特征**：“最后5单”、“明天恢复原价”、“不是谁都有资格抢”。
    - **目的**：临门一脚，促成成交。
...
export const analyzeScript = async (
  ...
  const prompt = `
    ...
    任务：
    1. 诊断原始话术是否缺少了完整的成交闭环（拉新-塑品-保障-逼单）。
    2. **强制生成实战模拟**：生成的 steps 数组必须严格包含 4 个步骤，顺序为：
       - Step 1: 拉新/破冰 (必须用反向指令，如“别拍”)
       - Step 2: 塑品/价值
       - Step 3: 保障/信任 (售后/承诺)
       - Step 4: 逼单/收割 (稀缺性)
    3. 输出纯 JSON。
